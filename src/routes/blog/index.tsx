	...
	return (
		<div className="space-y-6 md:space-y-8 3xl:space-y-10 4xl:space-y-12 px-4 md:px-6 lg:px-8 3xl:px-12 4xl:px-16 py-8 md:py-12 3xl:py-16 4xl:py-24">
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 md:gap-4">
				<h1 className="text-2xl sm:text-3xl md:text-4xl 3xl:text-5xl 4xl:text-6xl font-pixel">BLOG POSTS</h1>
				<Link to="/blog/new">
					<PixelButton className="text-xs md:text-sm 3xl:text-base 4xl:text-lg">NEW POST</PixelButton>
				</Link>
			</div>
						...
						>
							<PixelCard hoverEffect className="h-full flex flex-col">
								...
								<div className="mt-4 md:mt-6 3xl:mt-8 pt-3 md:pt-4 border-t-2 md:border-t-3 4xl:border-t-4 border-muted flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-xs md:text-sm 3xl:text-base 4xl:text-lg font-pixel text-muted-foreground">
									...
									<span className="whitespace-nowrap">BY {post.authorName}</span>
								</div>
							</PixelCard>
						</Link>
					))
